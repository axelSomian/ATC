...
| **Actualité ATC — Lot 1 (fondations + admin)** | 🔴 Haute | 🚧 En cours (2026-09-02) | Modèle `Post` unique (tournoi/événement/partenariat/infos_tennis/atc ; statut draft→scheduled→published→archived ; migration `20260902200000_news`). Visibilité **calculée à la lecture** (`published` OU `scheduled` + `publishedAt` atteint) → aucune tâche planifiée (Render dort). Module API `modules/news` : public `/api/v1/news` (feed curseur, `/featured`, `/events/upcoming`, `/events`, `/partner-of-month`, `/:slug`) + admin `/api/v1/admin/news` (CRUD + upload image Cloudinary `atc/news`). Corps en **Markdown** rendu HTML assaini côté API (`lib/markdown.ts` : `marked` + `sanitize-html`). Push à tous à la publication (`sendBroadcast`, case admin). Onglet admin « Actualité ». **Reste (Lots 2-4)** : page publique `/actualite` (à la une carrousel, filtres catégories, feed, bandeau événements, partenaire du mois), page détail + galerie + CTA, entrée nav (widget accueil + sidebar). |
